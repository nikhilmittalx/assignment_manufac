...
import { getMaxMinCrop } from './utils/getMaxMinCrop';
import { getAvgYieldAndCultivation } from './utils/getAvgYieldAndCultivation';

function App() {

  const maxMinCrops = getMaxMinCrop().map((cropData, index) => (
  ...
  return (
    <>
      <Title order={1} className='title1'>Crops with maximum and minimum production</Title>
      <Table withTableBorder withColumnBorders id='Crop_maxmin'>
      {/* <Table striped highlightOnHover withTableBorder withColumnBorders id='Crop_maxmin'> */}
        <Table.Thead>
          <Table.Tr>
      ...
      <Title order={1} className='title2'>Average Yield and Cultivation of Crop</Title>
      <Table striped highlightOnHover withTableBorder withColumnBorders id="Crop_avg">
        <Table.Thead>
          <Table.Tr>
